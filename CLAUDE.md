...
A Terraform provider (built on the [terraform-plugin-framework](https://github.com/hashicorp/terraform-plugin-framework)) for managing [MisterShell](https://www.mistershell.com) inventory and configuration: locations, network resources, credentials, tags, roles, log destinations, application settings, session-policy ACLs/rules, external auth providers, workers, and AI models/prompts/agents/skills. This repo is the provider (client) only and talks to a running MisterShell instance over its REST API; the backend lives in a separate, private repository.

**Builtin objects.** Several entities ship with MisterShell-managed *builtin* objects the API protects from mutation: the bootstrap **default worker** (`is_default=true`, 403 on edit/delete), **system AI prompts** (`type="system"`, 403 on create/edit/delete), **builtin AI agents** (`type` prefixed `builtin_` — cannot be created/deleted, though the API *does* allow patching their model/prompt/config/tools), and **builtin AI skills** (`is_builtin=true` — only `is_enabled` is mutable). AI **tools** are entirely backend-builtin (read-only). The provider manages user-created objects; builtins are exposed read-only through data sources (the chosen scope — builtin agents/skills are not adopted as resources). The `ai_model` resource names its provider attribute **`model_provider`** because `provider` is a reserved Terraform argument. All list endpoints accept page `size` up to 10000 (the AI endpoints' former 1000 cap was lifted backend-side), and the client uses `size=10000` throughout.
...
- **Tag↔resource edges can be owned from either side, never both.** A tag's members are settable via `mistershell_tag.resource_ids` (tag-centric, `PUT /tags/{id}/assignments`); a resource's tags via `mistershell_resource.tag_ids` (resource-centric, `PUT /resources/{id}/tags`). Both are whole-list exact-replace. `mistershell_resource.tag_ids` uses **opt-in exclusive ownership**: null/omitted = the provider does not touch tags (so the tag side can own them); a set value (incl. `[]`) = the provider enforces that exact set. Both the resource and its data source also expose a computed `tags` read-back (list of `{id,name,color,description}`). The plural `mistershell_tags` data source lists the whole catalog. There is no granular single-edge endpoint, so a per-edge association resource is deliberately not provided.
- **Supported type lists are generated, not hardcoded.** `client.SupportedResourceTypes`, `client.SupportedCredentialTypes`, and the other `Supported*` slices (log-destination/syslog/webhook enums, auth-provider/rule/ACL enums, and `SupportedWorkerStatuses` / `SupportedAIModelProviders` / `SupportedAIPromptTypes` / `SupportedAIAgentTypes`) live in `internal/client/types_gen.go` and feed the corresponding `stringvalidator.OneOf(...)` validators. They are generated from the MisterShell OpenAPI spec (`ui/openapi.json` — e.g. `components.schemas.NetworkResourceType`, `CredentialType`, `WorkerStatus`, `AIModelProvider`, `AIPromptType`, `AIAgentType`) by `internal/gen/types` (`//go:generate` directive in `generate.go`). Do **not** re-hardcode these enums or hand-edit `types_gen.go`. Refresh with `make generate`, pointing `MISTERSHELL_OPENAPI` at a local checkout of the (private) backend's `ui/openapi.json` (there is no default git source). The generator fails loudly if either enum is missing/empty, guarding against an upstream schema rename.
